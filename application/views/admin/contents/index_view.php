                ...
                foreach($contents as $content_id => $content)
                {
                    ...
                    if(strlen($content['featured_image'])>0)
                    {
                        ...
                        echo ' '.anchor('admin/images/delete_featured/'.$content_id,'<span class="glyphicon
                            glyphicon-remove" aria-hidden="true"></span>','onclick="return confirm(\'Are you sure you want to delete?\')"', array('class' => 'btn btn-danger btn-sm'));
                    }
                    else
                    {
                        echo anchor('admin/images/featured/'.$content_id,'<span class="glyphicon glyphicon-plus"></span>', array('class' => 'btn btn-primary btn-sm'));
                    }
                    echo '</td>';
                    foreach($langs as $slug=>$language)
                    {
                        ...
                        {
                            echo anchor('admin/contents/edit/'.$slug.'/'.$content_id,'<span class="glyphicon glyphicon-pencil" aria-hidden="true"></span>', array('class' => 'btn btn-primary btn-sm'));
                            echo ' '.anchor('admin/contents/delete/'.$slug.'/'.$content_id,'<span class="glyphicon glyphicon-remove" aria-hidden="true"></span>','class="btn btn-danger btn-sm" onclick="return confirm(\'Are you sure you want to delete?\')"');
                            $rakestyle = '';
                        ...
                        {
                            echo anchor('admin/contents/create/'.$content['content_type'].'/'.$slug.'/'.$content_id,'<span class="glyphicon glyphicon-plus" aria-hidden="true"></span>', array('class' => 'btn btn-primary btn-sm'));
                        }
                        echo '</td>';
                    }
                    echo '<td>';
                    echo anchor('admin/contents/delete/all/'.$content_id,'<span class="glyphicon glyphicon-remove" aria-hidden="true"></span>','onclick="return confirm(\'Are you sure you want to delete?\')"', array('class' => 'btn btn-danger btn-sm'));
