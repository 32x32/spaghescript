...
class Menus {
    ...
    public function get_menu($menu_name, $language_slug = NULL, $return = 'array')
    {
        ...
        if(!empty($menus))
        {
                    ...
                    if(!empty($item->translations))
                    {
                        foreach ($item->translations as $translation)
                        {
                            if($translation->language_slug==$language_slug) {
                                ...
                                $the_menu[] = array('id'=>$translation->item_id, 'parent_id'=>$item->parent_id, 'title'=>$translation->title, 'url'=>$url, 'styling'=>$item->styling);
                            }
                        }
                    }
                }
            ...
            switch ($return) {
                ...
                case 'bootstraptor_menu': {
                    $the_menu = $this->bootstraptor_menu($the_menu);
                }
            }
            return $the_menu;
        }
        ...
        foreach($array as $element)
        {
            if($element['parent_id']==$parent_id)
            {
                if(in_array($element['id'],$parents))
                {
                    ...
                    $menu_html .= '<li>';
                    $menu_html .= '<a href="' . $element['url'] . '">' . $element['title'] . ' '. $element['styling'] .' </a>';
                }
                if(in_array($element['id'],$parents))
                {
    ...
    function bootstraptor_menu($array,$parent_id = 0,$parents = array())
    {
        if($parent_id==0)
        {
            foreach ($array as $element) {
        ...
        foreach($array as $element)
        {
            if($element['parent_id']==$parent_id)
            {
                ...
                {
                    $menu_html .= '<ul class="dropdown-menu" role="menu">';
                    $menu_html .= $this->bootstraptor_menu($array, $element['id'], $parents);
                    $menu_html .= '</ul>';
                }
